...
FUNC_WRAPPER(
	...
	"hDevice = %p, videoAlg = %u, FGTEnable = %u, MetaDataEnable = %u, Progressive = %u, OptFlags = 0x%08x");
...
FUNC_WRAPPER(
	...
	eprintf("  width = %u, height = %u, startCodeSz = %u, pMetaData = %p, metaDataSz = %u, bEnableScaling = %u,\n",
...
FUNC_WRAPPER(
	...
	eprintf("pOut = {\n");
	eprintf("    Ybuff = %p (YbuffSz = %u, YBuffDoneSz = %u), UVbuff = %p (UVbuffSz = %u, UVBuffDoneSz = %u),\n",
